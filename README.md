...
간격의 정체는 이 한 줄에서 보인다. 뮤테이션 테스팅이 같은 `<img>` 의 두 속성을
각각 망가뜨렸을 때 결과가 정반대였다.

```tsx
<img src={`${URL}/${imagePath}`} alt={`${name} scoop`} />
//       🔴 "" 로 바꿔도 28개 전부 통과    🟢 바꾸면 즉시 실패
```

**이미지 주소를 통째로 지워도 아무 테스트도 눈치채지 못한다.** 이걸 검증했어야 할
테스트의 이름은 `displays image for each scoop option from server`이고,
개수와 `alt`는 확인하지만 `src`는 확인하지 않는다. 그 파일의 커버리지는 **100%**다.

커버리지가 100%인 앱 컴포넌트 다섯 개의 뮤테이션 스코어는 **45%~91%로 흩어진다.**
커버리지 열은 상수라 이 다섯을 구분할 정보를 하나도 담고 있지 않다.

→ [`docs/02-커버리지-100퍼센트가-말해주지-않는-것.md`](docs/02-커버리지-100퍼센트가-말해주지-않는-것.md)
· [`docs/00-베이스라인-발견.md`](docs/00-베이스라인-발견.md)
...
npm run test:mutation # Stryker, 약 1분
```

커버리지 단독으로 "테스트가 좋아졌다"고 판정하지 않는다.
이 레포의 논지가 커버리지의 한계이므로 자기모순이 된다.
**커버리지가 같아도 뮤테이션 스코어가 낮으면 진 것이다.**
...
- [x] **3** — [Stryker 도입, 뮤테이션 스코어 75.00%](reports/baseline.md) · *커버리지와의 간극 22.62%p*
